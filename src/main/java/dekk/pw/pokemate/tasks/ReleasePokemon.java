...
public class ReleasePokemon implements Task {
    ...
    public void run(Context context) {
        ...
        for (List<Pokemon> list : groups.values()) {
            ...
            list.stream().filter(p -> Config.getAlwaysTransferPokemons().contains(p.getPokemonId().getNumber()) || (p.getCp() < Config.getMinCP() && list.indexOf(p) < list.size() - 1 && !p.getFavorite() && context.getIvRatio(p) < Config.getIvRatio() && !Config.getNeverTransferPokemons().contains(p.getPokemonId().getNumber()))).forEach(p -> {
